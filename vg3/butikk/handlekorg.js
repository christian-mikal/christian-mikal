...
class Handlekorg {
	...
	add(vare) {
		if (this.liste[vare.navn]) {
			this.liste[vare.navn].antall +=1;  // antall av denne varen
		} else {
			this.liste[vare.navn] = { "antall": 1, "vare": vare };
		}
		this.totAntall +=1;  // totalt antall varer
	}
	...
	vis(displayDiv, eventListener) {
		...
		//Denne listen er object, da bruker jeg in
		for (let varenavn in this.liste) {
			let item = this.liste[varenavn];
			//item vet antallet, mens varen vet navnet
